...
import { ShoppingCart, ShieldCheck, Zap, RefreshCw, ChevronRight, ChevronLeft, Sparkles, Flame, Tag, CheckCircle2 } from 'lucide-react';
...
export function Hero() {
  ...
  const featuredPhone = PHONE_PRODUCTS[0]; // iPhone 16 Pro Max

  const filteredPhones = PHONE_PRODUCTS.filter(phone => {
    if (activeBrandFilter === 'Apple') return phone.brand === 'Apple';
    if (activeBrandFilter === 'Samsung') return phone.brand === 'Samsung';
    if (activeBrandFilter === 'Android') return phone.brand !== 'Apple' && phone.brand !== 'Samsung';
    return true;
  });

  const visibleCardsCount = 3;
  const maxIndex = Math.max(0, filteredPhones.length - visibleCardsCount);

  const triggerSlideAnimation = (callback: () => void) => {
    setIsSliding(true);
    callback();
  ...
  return (
    <section id="hero" className="relative pt-44 sm:pt-48 pb-20 px-4 sm:px-6 bg-[#030712] overflow-hidden font-sans">
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10">
        ...
        {/* Top Hero Grid: Title Left, Product Showcase Right */}
        <div className="grid lg:grid-cols-12 gap-8 items-center">
          
          {/* Left 5 Cols: Copywriting & Badges */}
          <div className="lg:col-span-5 space-y-6 text-left animate-fade-in">
            ...
            <h1 className="text-4xl sm:text-6xl font-black text-white tracking-tight uppercase leading-[1.1]">
              ...
              <span className="liquid-text-gradient">LIQUID GLASS 2026</span>
            </h1>
          ...
          {/* Right 7 Cols: Showcase Product Frame */}
            ...
            <div className="relative rounded-3xl p-3 liquid-glass overflow-hidden group">
              <div className="relative h-[320px] sm:h-[440px] rounded-2xl overflow-hidden bg-slate-950/80 flex items-center justify-center p-6 border border-white/10">
                ...
                <div className="absolute bottom-5 left-6 liquid-glass px-5 py-3 rounded-2xl text-left shadow-2xl">
                  ...
                  <h3 className="text-xl font-black text-white">{featuredPhone.name}</h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-sm font-black text-amber-400">
                      {featuredPhone.storages[0].price.toLocaleString('vi-VN')} đ
                    </span>
                    <span className="text-xs text-slate-400 line-through">
                      {(featuredPhone.storages[0].price + 2000000).toLocaleString('vi-VN')} đ
                    </span>
                  </div>
                </div>
        ...
        {/* Bottom Section: Top Best Sellers Carousel Track */}
        <div className="pt-12 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            {/* Brand Filter Tabs & Spring Arrow Controls */}
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
              <button
                onClick={() => { setActiveBrandFilter('All'); setCarouselIndex(0); }}
                className={`px-4 py-2 rounded-xl text-xs font-black spring-tab cursor-pointer whitespace-nowrap ${
                  activeBrandFilter === 'All'
                    ? 'liquid-button text-slate-950 shadow-md'
              ...
              {/* Spring Carousel Arrow Buttons */}
              <div className="flex items-center gap-1.5 pl-2 border-l border-white/10">
                <button
                  onClick={prevSlide}
                  className="w-10 h-10 rounded-xl liquid-card flex items-center justify-center text-slate-300 hover:text-white hover:border-cyan-400 spring-tab active:scale-90 cursor-pointer shadow-lg"
                  title="Xem máy trước"
                >
          ...
          {/* Cards Grid with Smooth Slide Animation */}
          <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 transition-all duration-300 ${isSliding ? 'opacity-40 scale-98 translate-x-2' : 'opacity-100 scale-100 translate-x-0'}`}>
            {filteredPhones.slice(carouselIndex, carouselIndex + visibleCardsCount).map((phone) => (
              <div
                key={phone.id}
                className="liquid-card rounded-3xl p-6 space-y-5 flex flex-col justify-between group"
              >
                ...
                <div className="h-52 w-full rounded-2xl overflow-hidden bg-slate-950/80 p-4 relative flex items-center justify-center border border-white/5">
                  ...
                  <div className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-amber-500 text-slate-950 text-[10px] font-black uppercase shadow-md flex items-center gap-1">
                    <Tag className="w-3 h-3" /> Trợ giá {(phone.tradeInBonus / 1000000).toFixed(1)}Tr
                  </div>
                </div>
